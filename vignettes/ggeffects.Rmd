...
**ggeffects** computes marginal effects at the mean (MEM) or at representative values (MER) from statistical models and returns the result as tidy data frame, especially for further use with [ggplot](https://cran.r-project.org/package=ggplot2). Definitions [can be found here](https://stats.stackexchange.com/tags/marginal-effect/info).
...
Finally, a third grouping structure can be defined, creating another column named `panel`. In such cases, you may create multiple plots (for each value in `panel`). **ggeffects** takes care of this when you use `plot()` and automatically creates an integrated plot with all panels in one figure.
...
By default, `x` is numeric, or put differently, printing the results will cause factors as `x` to become numeric...
...
```{r}
ggpredict(m, "c172code", x.as.factor = FALSE)
```
